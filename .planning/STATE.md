...
| Phase 09-task-database P03 | 5 | 1 tasks | 1 files |
| Phase 09-task-database P02 | 15 min | 1 tasks | 1 files |
| Phase 10-latex-export P02 | 5 min | 3 tasks | 4 files |
| Phase 10-latex-export P01 | 7 min | 2 tasks | 4 files |
...
- [Phase 09-task-database]: applyTaskFilter accepts both snake_case and camelCase task fields for test+API compatibility
- [Phase 09-task-database]: sortTaskRows uses { field, direction } signature matching RED test contracts; dueDate/due_date normalized in component
- [Phase 09-task-database]: TaskListView filter+column state stored in localStorage keyed by projectId for per-project persistence
- [Phase 09-task-database]: Two separate DndContext IDs (task-card-dnd / task-column-dnd) avoids sensor collision per research pitfall 3
- [Phase 09-task-database]: Optimistic update + rollback pattern for cross-column card drag: local state updates instantly, API persists, error reverts
- [Phase 10-latex-export]: Left-click on citation chip opens context menu (not navigation) — distinct from WikiLink which navigates on click
- [Phase 10-latex-export]: citationKeys.js created in Plan 02 (not Plan 01) as Rule 3 auto-fix; mirrors bibtex_service.py _make_citation_key()
- [Phase 10-latex-export]: Single-pass regex used for escapeLatex to prevent double-escaping when backslash replacement produces {} braces
- [Phase 10-latex-export]: deduplicateKeys: second collision gets 'b' suffix (chr(96+2)) matching Python chr(96 + seen_keys[key]) where count starts at 1
- [Phase 10-latex-export]: htmlToLatex strips only leading whitespace to preserve paragraph double-newlines in LaTeX output
- [Phase 10-latex-export]: NeurIPS conference papers produce @inproceedings (not @article) in generateBibContent
- [Phase 11-ai-experiment-gap-analysis]: GapAnalysisOutput uses plain BaseModel (not CamelModel) since it is agent output only, not a JSON response model — camelCase serialization happens at router layer via model_dump(by_alias=True)
...
- Task database uses `task_columns` table with stable ID PK; column_id (not display name) used as key in task field values — prevents rename corruption (research pitfall 2)
- Migration number 021 claimed for task database; range 021-025 reserved for v1.1 — prevents numbering collision (research pitfall 6)
- LaTeX serializer uses @tiptap/static-renderer nodeMapping (structured AST) not regex-on-HTML — chosen at roadmap time to prevent full-rewrite-level mistake (research pitfall 1)
- Gap analysis suggestions held in transient React useState only — not persisted to DB; re-running is cheap; matches NotesCopilotPanel suggestion pattern
- Gap analysis prompt must include full experiment tree (name, status, config, metrics), not just RQs — prevents LLM suggesting already-completed experiments (research pitfall 4)
- Suggestion cards are editable before promotion — not binary accept/reject; researcher modifies config/name before drag-to-create (research pitfall 6)
...
- Validate tiptap table node JSON structure on Phase 10 day one (dump table node via editor.getJSON() to confirm ProseMirror schema before committing to \begin{tabular} mapping)
- Research-phase recommended before Phase 11 implementation: gap analysis prompt engineering and context serialization strategy for large experiment trees

### Blockers/Concerns

- Task database schema: use task_columns table with stable ID PK and column_id FK in task field values from migration day one — retrofitting is a destructive migration
- LaTeX serializer choice: @tiptap/static-renderer nodeMapping must be the implementation path — any regex-on-HTML approach requires a full rewrite; decide before writing any serializer code
- Gap analysis context budget: experiment trees with 100+ experiments may exceed 4,000 token target; address in Phase 11 research-phase before committing to planning board UI
...
| 7 | Address 6 tech debt items from v1.0 audit | 2026-03-18 | c8772f3 | [7-address-6-tech-debt-items-from-v1-0-mile](./quick/7-address-6-tech-debt-items-from-v1-0-mile/) |
